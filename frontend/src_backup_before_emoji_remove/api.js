...
async function request(path, options = {}) {
  ...
  for (const baseUrl of BASE_URLS) {
    try {
      ...
      if (res.status === 502 || res.status === 503) {
        // Server proxy issue, try next target URL
        continue;
      }
      ...
      if (err.message && !err.message.includes("Cannot connect") && !err.message.includes("502") && !err.message.includes("503") && !err.message.includes("Failed to fetch")) {
        // Application error (e.g. 400 Bad Request, 401 Unauthorized), rethrow immediately
        throw err;
      }
      lastError = err;
    }
  ...
  throw lastError || new Error("Cannot connect to server. Please ensure backend is running at http://127.0.0.1:8000.");
}
