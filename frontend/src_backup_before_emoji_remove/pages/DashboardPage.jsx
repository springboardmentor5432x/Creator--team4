...
export default function Dashboard({ user, onBack }) {
  ...
  // â”€â”€ AUTOMATICALLY FETCH CONNECTED YOUTUBE CHANNEL METRICS â”€â”€
  useEffect(() => {
  ...
  // â”€â”€ DYNAMIC ANALYTICS CALCULATIONS FROM REAL CONNECTED ACCOUNTS â”€â”€
  const getConnectedYtStats = () => {
    ...
    return { subs: 4850000, views: 680000000, videos: 940 };
  };
  ...
  const getConnectedLiStats = () => {
    ...
    return { conn: 36500000, impr: 4200000 };
  };
  ...
  const getConnectedIgStats = () => {
    ...
    if (folVal > 0) return { followers: folVal, posts: postVal || 3740, eng: parseFloat(connectedInstagramEngagement || 3.45) };
    return { followers: 638000000, posts: 3740, eng: 3.45 };
  };
  ...
  const getConnectedFbStats = () => {
    ...
    if (folVal > 0) return { followers: folVal, reach: reachVal || folVal * 5, eng: parseFloat(connectedFacebookEngagement || 4.2) };
    return { followers: 28500000, reach: 142000000, eng: 4.2 };
  };
  ...
  const getConnectedTwitterStats = () => {
    ...
    if (folVal > 0) return { followers: folVal, tweets: tweetsVal || 4850, impr: folVal * 18, eng: parseFloat(connectedTwitterEngagement || 6.8) };
    return { followers: 31200000, tweets: 4850, impr: 561600000, eng: 6.8 };
  };
  ...
  // â”€â”€ PROFILE-SPECIFIC TAILORED ANALYTICS GENERATORS â”€â”€
  ...
  // â”€â”€ CRITICAL FIX: useState lazy initializers only run once at mount, so they
  ...
  // Connect LinkedIn via Handle or Profile URL
  const handleConnectDirectLinkedin = async (inputVal) => {
    const val = (inputVal || linkedinInput).trim();
    if (!val) return;
    setLiConnecting(true);
    setError('');
    try {
      const data = await api.connectLinkedin(val, window.location.origin);
      if (data && data.user) {
  ...
  // Connect LinkedIn â€” Redirects user to real LinkedIn Login page
  ...
  const handleConnectInstagram = async (username) => {
    ...
    try {
      ...
      setConnectedInstagramId(u.instagram_profile_id || `ig_${username.toLowerCase()}`);
      setConnectedInstagramTitle(u.instagram_profile_title || username);
      setConnectedInstagramPicture(u.instagram_profile_picture || null);
      setConnectedInstagramFollowers(u.instagram_followers_count || 0);
      setConnectedInstagramEngagement(u.instagram_engagement_rate || 0.0);
      setConnectedInstagramPosts(u.instagram_posts_count || 0);
      setConnectedInstagramVerifiedMeta(u.instagram_verified_meta || false);

      setLoadingIgMeta(true);
      try {
        const analytics = await api.getInstagramAnalytics();
        setMetaIgPosts(analytics.posts || []);
      } catch (_) {}
      setLoadingIgMeta(false);

      if (data && data.warning) {
        setError('âš ï¸ ' + data.warning);
      }
    } catch (err) {
  ...
  const handleConnectFacebook = async (pageName, groupId) => {
    ...
    try {
      ...
      setConnectedFacebookId(u.facebook_page_id || `fb_${(pageName || groupId).toLowerCase()}`);
      setConnectedFacebookTitle(u.facebook_page_title || pageName || groupId);
      setConnectedFacebookPicture(u.facebook_page_picture || null);
      setConnectedFacebookFollowers(u.facebook_followers_count || 0);
      setConnectedFacebookReach(u.facebook_reach_count || 0);
      setConnectedFacebookEngagement(u.facebook_engagement_rate || 0.0);
      setConnectedFacebookVerifiedMeta(u.facebook_verified_meta || false);

      if (data && data.warning) setError('âš ï¸ ' + data.warning);
  ...
  const handleConnectTwitter = async (username) => {
    ...
    try {
      ...
      const u = (data && data.user) ? data.user : (data || {});
      try { localStorage.setItem('creatoriq_user', JSON.stringify(u)); } catch (e) {}
      setConnectedTwitterUsername(u.twitter_username || username);
      setConnectedTwitterDisplayName(u.twitter_display_name || username.title ? username : username);
      setConnectedTwitterPicture(u.twitter_profile_picture || null);
      setConnectedTwitterFollowers(u.twitter_followers_count || 0);
      setConnectedTwitterFollowing(u.twitter_following_count || 0);
      setConnectedTwitterTweets(u.twitter_tweets_count || 0);
      setConnectedTwitterEngagement(u.twitter_engagement_rate || 0.0);
      setConnectedTwitterVerified(u.twitter_verified || false);
      if (data && data.warning) setError('âš ï¸ ' + data.warning);
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            ...
            {[
              { id: 'agency', label: 'Agency Dashboard', roles: ['Creator', 'Agency', 'Marketing Team', 'Administrator'], svg: <path stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 0 0-2-2H7a2 2 0 0 0-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v5" /> },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', borderTop: '1px solid var(--border-color)', paddingTop: '1.5rem' }}>
          ...
          >
            {user.role === 'Administrator' ? 'â† Admin Panel' : 'Sign Out'}
          </button>
        </div>
      </div>
      ...
      <div className="print-main-content" style={{ flexGrow: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto', background: 'var(--bg-color)' }}>
        ...
        {(user.role === 'Agency' || user.role === 'Administrator') && activeTab !== 'agency' && (
          <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: '13px', color: 'var(--text-primary)', fontWeight: '600' }}>
                  {selectedAgencyCreatorId === 'all' 
                    ...
                    : `Filtered for creator: ${selectedAgencyCreatorObject?.creator_name} (@${selectedAgencyCreatorObject?.handle}) â€¢ ${selectedAgencyCreatorObject?.category}`}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              >
                <option value="all">ðŸŒ All Managed Creators (Roster Aggregate)</option>
                {agencyCreators.map(c => (
                  <option key={c.id} value={c.id}>
                    ðŸ‘¤ {c.creator_name} (@{c.handle}) â€¢ {c.category}
                  </option>
                ))}
              </select>
            </div>
                              ...
                              return (
                                <tr key={idx} style={{ borderBottom: '1px solid var(--border-color)', transition: 'background-color 0.2s' }}>
                                  ...
                                  <td style={{ padding: '0.875rem 1rem', textAlign: 'right', fontWeight: 600, color: isWinner ? 'var(--emerald-400)' : 'var(--rose-400)' }}>
                                    {isWinner ? (
                                      <span>ðŸ† +{row.format(Math.abs(diff))} ahead</span>
                                    ) : (
                          ...
                          <tbody>
                            {ytData.videos.map(v => (
                              <tr key={v.id} style={{ borderBottom: '1px solid var(--border-color)', transition: 'background-color 0.2s' }}>
                                ...
                                <td style={{ padding: '1rem 1.25rem', textAlign: 'right', color: 'var(--indigo-400)' }}>{(v.comments ?? 0).toLocaleString()}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflowY: 'auto', flexGrow: 1 }}>
            {!connectedLinkedinId ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 2rem', textAlign: 'center', flexGrow: 1, animation: 'fadeIn 0.3s ease' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%', maxWidth: '28rem', marginBottom: '1.5rem' }}>
                  <div style={{ display: 'flex', gap: '0.5rem', width: '100%' }}>
                    <input
                      type="text"
                      placeholder="e.g. williamhgates or satyanadella"
                      value={linkedinInput}
                      onChange={(e) => setLinkedinInput(e.target.value)}
                      onKeyDown={(e) => e.key === 'Enter' && handleConnectDirectLinkedin()}
                      style={{
                        flexGrow: 1,
                        padding: '0.75rem 1rem',
                        borderRadius: '0.75rem',
                    ...
                    />
                    <button
                      onClick={() => handleConnectDirectLinkedin()}
                      disabled={liConnecting || !linkedinInput.trim()}
                      style={{
                        background: '#0077b5',
                        color: '#ffffff',
                        border: 'none',
                        ...
                        padding: '0.75rem 1.25rem',
                        fontWeight: 700,
                        cursor: (liConnecting || !linkedinInput.trim()) ? 'not-allowed' : 'pointer',
                        fontSize: '0.9rem',
                        boxShadow: '0 4px 12px rgba(0,119,181,0.35)',
                        opacity: (liConnecting || !linkedinInput.trim()) ? 0.6 : 1,
                        whiteSpace: 'nowrap'
                      }}
                    >
                      {liConnecting ? 'Connecting...' : 'Connect Profile'}
                    </button>
                  </div>
                  ...
                  >
                    <svg style={{ width: '1.25rem', height: '1.25rem', fill: '#00a0dc' }} viewBox="0 0 24 24">
                      <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.779-1.75-1.75s.784-1.75 1.75-1.75 1.75.779 1.75 1.75-.784 1.75-1.75 1.75zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                    </svg>
                    {liConnecting ? 'Redirecting...' : 'Sign in with LinkedIn OAuth'}
                  </button>
                </div>
              </div>
            ) : (
                    ...
                    {[
                      ...
                      <div key={idx} style={{ paddingBottom: idx < 2 ? '1rem' : '0', borderBottom: idx < 2 ? '1px solid var(--border-color)' : 'none', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1.5rem', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                          <span>ðŸ‘ï¸ <strong>{post.impressions}</strong> Impressions</span>
                          <span>ðŸ‘ <strong>{post.interactions}</strong> Interactions</span>
                        </div>
                      </div>
                    ))}
                  </div>
                        ...
                        <div key={post.id} style={{ background: 'var(--card-muted-bg)', border: '1px solid var(--border-color)', borderRadius: '0.75rem', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                          ...
                          <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem', flexGrow: 1 }}>
                            ...
                            <div style={{ marginTop: 'auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.75rem', color: 'var(--text-muted)', paddingTop: '0.5rem' }}>
                              ...
                              <span>ðŸ’¬ {post.comments_count || 0} Comments</span>
                            </div>
                            {post.permalink && (
                    ...
                    <div style={{ background: 'rgba(59,130,246,0.05)', border: '1px solid rgba(59,130,246,0.15)', borderRadius: '0.75rem', padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
                        <span style={{ fontSize: '0.875rem', fontWeight: 700, color: 'var(--text-primary)' }}>
                          Instagram Connected â€¢ @{connectedInstagramTitle || 'User'}
                        </span>
                      ...
                      {fbPosts.map(post => (
                        <div key={post.id} style={{ background: 'var(--card-muted-bg)', border: '1px solid var(--border-color)', borderRadius: '0.75rem', padding: '1rem 1.25rem' }}>
                          ...
                          <div style={{ display: 'flex', gap: '1.5rem', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                            <span>ðŸ‘ {post.reactions_count}</span>
                            <span>ðŸ’¬ {post.comments_count}</span>
                            {post.url && <a href={post.url} target="_blank" rel="noopener noreferrer" style={{ color: '#1877f2', textDecoration: 'none', marginLeft: 'auto' }}>View on Facebook â†’</a>}
                          </div>
                        </div>
                      ))}
                      {fbVideos.map(video => (
                        <div key={video.id} style={{ background: 'var(--card-muted-bg)', border: '1px solid var(--border-color)', borderRadius: '0.75rem', padding: '1rem 1.25rem', display: 'flex', gap: '1rem' }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', flexGrow: 1 }}>
                            ...
                            {video.url && <a href={video.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.75rem', color: '#1877f2', textDecoration: 'none', marginTop: 'auto' }}>Watch Video â†’</a>}
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {!connectedTwitterUsername ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 2rem', textAlign: 'center', flexGrow: 1, animation: 'fadeIn 0.3s ease' }}>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', maxWidth: '28rem', lineHeight: 1.6, marginBottom: '2.5rem', background: 'rgba(29,161,242,0.07)', border: '1px solid rgba(29,161,242,0.15)', borderRadius: '0.75rem', padding: '0.75rem 1rem' }}>
                  â„¹ï¸ Connected to the <strong>Official Twitter Developer API v2</strong>. Fetching live profile data and recent tweet activity directly from X.
                </p>
                {error && (
              ...
              <div style={{ padding: '2.5rem', display: 'flex', flexDirection: 'column', gap: '2rem', animation: 'fadeIn 0.4s ease', flexGrow: 1, textAlign: 'left' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '1.5rem', flexWrap: 'wrap', gap: '1.5rem' }}>
                  <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', margin: '0.25rem 0 0 0' }}>@{connectedTwitterUsername} Â· Twitter/X Analytics</p>
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: '0.75rem' }}>
                ...
                <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border-color)', borderRadius: '1rem', padding: '1.5rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
                    ...
                    <a href={`https://twitter.com/${connectedTwitterUsername}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.8rem', color: '#1da1f2', textDecoration: 'none', fontWeight: 600 }}>View on X â†’</a>
                  </div>
                            ...
                            <a key={tweet.id} href={tweet.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', color: 'inherit', display: 'flex', padding: '1.25rem', borderBottom: '1px solid #eff3f4', gap: '0.75rem', cursor: 'pointer' }}>
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', flexWrap: 'wrap' }}>
                                  ...
                                  {connectedTwitterVerified && <span style={{ color: '#1d9bf0', fontSize: '1.1rem', lineHeight: 1 }}>â˜‘ï¸</span>}
                                  <span style={{ color: '#536471', fontSize: '0.9rem' }}>@{connectedTwitterUsername}</span>
                                  <span style={{ color: '#536471', fontSize: '0.9rem' }}>Â· {new Date(tweet.created_at).toLocaleDateString(undefined, {month: 'short', day: 'numeric'})}</span>
                                </div>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#536471', marginTop: '0.75rem', fontSize: '0.85rem', maxWidth: '300px' }}>
                                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>ðŸ’¬ {tweet.reply_count > 0 ? formatNumber(tweet.reply_count) : ''}</span>
                                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>ðŸ”„ {tweet.retweet_count > 0 ? formatNumber(tweet.retweet_count) : ''}</span>
                                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>â¤ï¸ {tweet.like_count > 0 ? formatNumber(tweet.like_count) : ''}</span>
                                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>ðŸ”—</span>
                                </div>
                              </div>
                            </a>
                          ))}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '0.7rem', color: '#8b9bb4' }}>Sponsored â€¢ ðŸŒ</div>
                        </div>
                      </div>
                      <p style={{ fontSize: '0.8rem', color: '#e2e8f0', margin: 0, whiteSpace: 'pre-wrap', lineHeight: 1.4 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '0.7rem', color: '#8b9bb4' }}>Community Post â€¢ Just now</div>
                        </div>
                      </div>
                      <p style={{ fontSize: '0.8rem', color: '#e2e8f0', margin: 0, whiteSpace: 'pre-wrap', lineHeight: 1.4 }}>
                          ...
                          <tr key={post.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                            ...
                            <td style={{ padding: '1rem', textAlign: 'center' }}>
                              <div style={{ display: 'flex', gap: '0.35rem', justifyContent: 'center' }}>
                                {(post.selected_platforms || post.platforms || []).map(pf => (
        ...
        {/* ==================== MODULE 4 â€“ GROWTH & TREND ANALYSIS & MARKETING WORKSPACE ==================== */}
        {activeTab === 'reports' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
