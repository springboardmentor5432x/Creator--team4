...
export default function ConnectAccountsModal({ isOpen, onClose, onAccountUpdated }) {
  ...
  const handleConfirmAuth = async () => {
    ...
    try {
      ...
      const res = await api.handleOAuthCallback(platId, 'demo_auth_code_xyz', usernameParam);
      if (res && res.user) {
        try {
          localStorage.setItem('creatoriq_user', JSON.stringify(res.user));
        } catch (e) {}
      }
            ...
            <div className="bg-slate-800/80 border border-indigo-500/30 rounded-xl p-5 space-y-4 animate-scaleUp">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={
                    oauthModalData.platform.id === 'instagram' ? 'e.g. leomessi, codewithharry, your_handle' :
                    oauthModalData.platform.id === 'facebook' ? 'e.g. Meta, CodeWithHarry, your_page' :
                    oauthModalData.platform.id === 'linkedin' ? 'e.g. williamhgates, biswajitsahoo, profile URL' :
                    oauthModalData.platform.id === 'youtube' ? 'e.g. CodeWithHarry, Apna College' :
                    `e.g. ${oauthModalData.platform.id}_official`
                  }
